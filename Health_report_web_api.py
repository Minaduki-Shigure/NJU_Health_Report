...
def application(environ, start_response):
    ...
    # 获取get中key为name的值
    UserAuthCookie = params.get('UserAuthCookie', [''])[0]
    UserLocation = params.get('UserLocation', [''])[0]

    UserLastPCR = (datetime.datetime.now()).strftime("%Y-%m-%d %H")
    
    if (UserAuthCookie == ''):
        print('Error: UserAuthCookie is empty!')
        start_response('400 Bad Request', [('Content-Type', 'text/html'), ('Access-Control-Allow-Origin', '*')])
        return ['Error: UserAuthCookie is empty!'.encode()]

    if (UserLocation == ''):
        print("Error: No Location Provided. Using default.")
    ...
    try:
        ...
        print('Error: ' + str(e))
        start_response('500 Internal Error', [('Content-Type', 'text/html'), ('Access-Control-Allow-Origin', '*')])
        return [str(e).encode()]
    ...
    print('UserAuthCookie: ' + UserAuthCookie)
    print('UserLocation: ' + UserLocation)
    print('UserLastPCR: ' + UserLastPCR)
    print('Report result: ' + returnMsg)

    return [returnMsg.encode()]
